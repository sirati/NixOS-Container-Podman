      ...
      case "$subcmd" in
        ...
        develop)
          case "$cur" in
            -*)
              COMPREPLY=( $(compgen -W \
                "-A --forward-agent --x11 --x11-untrusted --wayland --wprs --dbus -S --socket --template --share --host-port --agent -D --develop-arg --mount-bashrc --mount-gitconfig --native --no-native" \
                -- "$cur") )
              ;;
            *)
